...
def log_file():
    yield
    time = "{}".format(datetime.now().strftime("%H:%M:%S.%f"))
    list = getout(f"ls -s {data['folder_in']}").replace('\n',', ')
    proc = getout("cat /proc/loadavg")
    string = 'Time: ' + time + ' | Files: ' + list + ' | Load: ' + proc
    getout(f"echo '{string}' >> /home/user/stat.txt")
